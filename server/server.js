...
import { getPosts, getPost, getTags, postRating, postPost, getNotifications } from './db/controllers/post';
import { getComments, getConversations, getMessages, postComment, postMessage } from './db/controllers/comment';
import { handleErrors } from './util/misc';
import { NOT_LOGGED_IN } from './db/types';
import User from './db/models/User';
import { setDisplayName, getUser, block, unblock, follow, unfollow, updateProfile, checkDisplayName } from './db/controllers/user';
...
const getUserInfo = (req, res, next) => {
  if (req.user)
    User.findById(req.user, (e, u) => {
      if (u) res.locals.user = { displayName: u.displayName, firstName: u.firstName, lastName: u.lastName, id: u.id };
      next();
    })
  else next();
}
...
const authenticate = (req, res, next) => {
  if (res.locals && res.locals.user)
    if (!res.locals.user.displayName) return res.json(handleErrors("No display name", {
      errors: {
        displayName: {
          message: 'You have to first set your display name on your profile!'
        }
      }
    }))
    else next();
  else return res.json(handleErrors(NOT_LOGGED_IN,
    { errors: { user: { message: 'You should be logged in for this feature!' } } }))
}
...
// Webpack
if (process.env.NODE_ENV !== 'production') {// Use webpack-dev-middleware if in development mode
