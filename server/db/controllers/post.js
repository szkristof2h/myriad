...
import { GET_POST_ERROR, POST_NOT_FOUND, INVALID_RATING, POST_POST_ERROR, GET_POSTS_ERROR, GET_TAGS_ERROR, GET_NOTIFICATIONS_ERROR } from '../types';
...
export function getPost(req, res) {
  ...
  Post
    ...
    .exec()
    .then(p => {
      if (!p) return Promise.reject({ error: { id: { message: 'No post with this id!' } } });
...
export function getPosts(req, res) {
  ...
  getDBPosts(criteria, 0, [], offset)
    ...
    .then(r => {
      posts = { ...posts, ...toObject(r, '_id'), ids: [...posts.ids, ...r.map(p => p._id)] };
      offset = getOffset(r.length, offset, 0);
    ...
    .then(r => {
      posts = {
        ...
        ids: [
          ...posts.ids,
          ...r.map(p => p._id),
          ...new Array(limits.reduce((a, v) => a + v) - posts.ids.length - r.length).fill('').map(_ => makeId())
        ]
      };
      
      if (posts.ids.length > 0 && userId) return Rating.find({ user: userId }).in('post', posts.ids.filter(id => id.length !== 20)).exec();
      return;
    }).then(r => {
...
// Search the db for post that's in one of the 3 rating tiers and not already chosen, then choose randomly from them
...
export function getNotifications(req, res) {
  ...
  FollowedList
    .find({ from: userId })
    .exec()
    .then(follows => follows.length == 0 ? false : 
      Post
        .find()
    ...
    .then(posts => res.json(posts ? { ...toObject(posts, '_id'), ids: posts.map(p => p._id) } : { ids: []}))
...
export function postRating(req, res) {
  ...
  if (![-1, 1].includes(rating)) return res.json(handleErrors(INVALID_RATING, {
  ...
  Post
    ...
    .exec()
    .then(p => {
      if (!p) return Promise.reject({ errors: { message: { id: 'No post with this id!'}} });
      post = p;
      return Rating.findOne({ post: _id, user: userId }).select('value').exec();
    }).then(r => {
      if (r) {
...
export function postPost(req, res) {
  const tags = !req.body.tags || !req.body.tags.includes(',') ||
    req.body.tags.split(',').filter(t => t.replace(/\//g, '')).length < 3 ?
    'You should give the post at least 3 tags (seperated by commas)!' :
    req.body.tags.split(',').map(t => t.toLowerCase().trim().replace(/\//g, ''));
  const userId = res.locals.user.id;
  const displayName = res.locals.user.displayName;
  const link = `http${req.body.link.includes('https://') ? 's' : ''}://www.${req.body.link.replace('http://', '')
